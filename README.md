...
### Task 2 — FRFCFS vs FCFS: row-buffer hit rate collapse (expected) vs actual result

Added a genuine `FCFS` scheduler class (`src/dram_controller/impl/scheduler/fcfs_scheduler.cpp`) since this codebase only ships `FRFCFS` — no plain FCFS existed to select via config. Implements the same interface with `compare()` reduced to pure arrival-order comparison, dropping FRFCFS's row-ready-first branch entirely. Registered as `impl: FCFS`; wired into the build via `CMakeLists.txt`. Patch: `part_d_ramulator/patches/fcfs_scheduler_addition.patch`.

**Result: no observable difference.** `memory_system_cycles`, `avg_read_latency_0`, and `row_hits_0`/`row_misses_0` are identical between FRFCFS and FCFS runs on the baseline trace — verified three times, including a byte-for-byte diff of the full per-cycle command trace from the `TraceRecorder` plugin (101 lines, `diff` exit code 0, zero differences). This is the opposite of the assignment's expected outcome ("row-buffer hit rate will collapse").

One early diff attempt *did* show a large difference (233 vs 101 command-trace lines, with FRFCFS repeatedly hitting an open row via `WR` and FCFS forced into frequent `PRE`→`ACT` cycles) — but on investigation this was comparing a stale leftover trace file from earlier in the build/debug process against a fresh FCFS run, not two genuinely comparable runs. Deleting both trace files and regenerating fresh from the current binary and configs eliminated the discrepancy entirely. Flagged here because it's a real methodological trap: always regenerate all comparison artifacts from a clean state before trusting a diff.

**Why the two schedulers converge for this workload**: traced the controller's request-dispatch logic (`generic_dram_controller.cpp::schedule_request()`). The read/write buffer — the only place `IScheduler::compare()` (where FRFCFS's row-hit-priority logic lives) is actually invoked — is checked *last*, after the active-request buffer and the priority buffer. `compare()` can only produce a result different from plain FCFS when it's asked to choose between two or more genuinely ready candidates at the same instant; if this buffer's effective concurrency is 0 or 1 whenever `get_best_request()` is called for this trace's request rate and this controller's per-cycle service rate, FRFCFS's row-hit-first branch never triggers, and both schedulers converge to the same decision by construction — not by coincidence. This wasn't verified with direct instrumentation (would require adding a debug counter inside `compare()` itself), so it's presented as the best-evidenced explanation from the traced code path, not a confirmed measurement.

**What this actually demonstrates**, in place of the assignment's expected row-hit-rate collapse: scheduler policy differences are workload-dependent — they only manifest under genuine request contention. A trace with a higher request arrival rate relative to this single-channel controller's per-cycle service capacity (denser bursts, more simultaneous cache-line misses) would be needed to create the multi-candidate scheduling decisions FRFCFS is actually designed to exploit. This is itself a legitimate finding about the tRCD/tRP-driven cost the assignment asks about: that cost is real and mechanistically present in the codebase (confirmed via the `PRE`/`ACT` command sequences visible when we compared the — now known to be invalid — stale trace), but this specific synthetic workload doesn't create the conditions to observe it. A follow-up with a denser, more contended trace would be the natural next step.
